...
async function signup(req, res) {
    try {
        ...
        if (!userId)
            return res.status(500).json({ status: "error", message: "Failed to create auth user" });
        ...
        const profile = await prisma_1.prisma.profile.create({
            ...
            select: { id: true, userId: true, email: true, username: true, firstName: true, lastName: true, fullName: true
            }
        });
        const code = (0, generateOtp_1.generateOtpCode)(6);
        const codeHash = (0, generateOtp_1.hashOtp)(email, code);
        const ttlMin = Number(process.env.OTP_TTL_MINUTES ?? 10);
        const expiresAt = new Date(Date.now() + ttlMin * 60 * 1000);
        await prisma_1.prisma.emailOtp.updateMany({
...
async function checkEmail(req, res) {
    try {
        ...
        if (existingProfile)
            return res.json({ status: "success", message: "Email is already in use! try another email.", available: false });
...
async function checkUsername(req, res) {
    try {
        ...
        if (existingProfile)
            return res.status(409).json({ status: "failed", message: "Username is already in use! try another email.", available: false });
    ...
    try {
        ...
        const result = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (Array.isArray(body.appInterests)) {
                ...
                const rows = await Promise.all(names.map(async (name) => tx.appInterest.upsert({
                    where: { name },
                    create: { name },
                    update: {},
                    select: { id: true },
...
async function signIn(req, res) {
    ...
    if (error)
        res.status(409).json({ message: "something went wrong signing in" });
...
async function verifyEmailOtp(req, res) {
    ...
    const now = new Date();
    const codeHash = (0, generateOtp_1.hashOtp)(email, code);
    const otp = await prisma_1.prisma.emailOtp.findFirst({
        where: {
            email,
            purpose,
            usedAt: null,
