...
exports.getMyCommentReaction = getMyCommentReaction;
...
async function createComment(req, res) {
    try {
        ...
        if (parentCommentId) {
            const parent = await prisma_1.prisma.comment.findUnique({
        ...
        if (post.profileId !== me.id) {
            await notification_service_1.NotificationService.comment(post.profileId, me.id, postId, comment.id);
        }
...
async function getCommentReplies(req, res) {
    try {
        ...
        const count = await prisma_1.prisma.comment.count({
        ...
        if (replies.length > limit)
            nextCursor = replies.pop()?.id ?? null;
...
async function reactToComment(req, res) {
    try {
        ...
        // 3) update type
        const reaction = await prisma_1.prisma.commentReaction.update({
        ...
        if (!isSelf) {
            ...
            if (!notif) {
                await notification_service_1.NotificationService.likeComment(comment.profileId, me.id, comment.postId, commentId);
            }
        }
...
async function getMyCommentReaction(req, res) {
    try {
        const me = await getCurrentProfile(req);
        if (!me)
            return res.status(401).json({ message: "Unauthenticated" });
        const { commentId } = req.params;
        const reaction = await prisma_1.prisma.commentReaction.findUnique({
            where: {
                commentId_profileId: {
                    commentId,
                    profileId: me.id
...
async function removeCommentReaction(req, res) {
    try {
        ...
        if (!me)
            return res.status(401).json({ message: "Unauthenticated" });
        const { commentId } = req.params;
        await prisma_1.prisma.commentReaction.deleteMany({
            where: {
                commentId,
                profileId: me.id
            }
        });
        const likeCount = await prisma_1.prisma.commentReaction.count({
            where: {
                commentId,
                type: "LIKE"
            }
