...
class ProfileController {
    static async getProfiles(req, res) {
        try {
            ...
            const friendSet = new Set(friendships.map((f) => (f.userAId === me.id ? f.userBId : f.userAId)));
            // 2) pending requests (both directions)
            const pending = await prisma_1.prisma.friendRequest.findMany({
                where: {
                    status: "PENDING",
                    OR: [
            ...
            const outgoingMap = new Map(); // profileId -> requestId
            const incomingMap = new Map(); // profileId -> requestId
            for (const r of pending) {
                if (r.requesterId === me.id)
                    outgoingMap.set(r.addresseeId, r.id);
                else
                    incomingMap.set(r.requesterId, r.id);
            }
            const items = profiles.map((p) => {
                if (friendSet.has(p.id)) {
                    return { ...p, friendStatus: "FRIENDS" };
                }
    ...
    static async getProfileById(req, res) {
        try {
            ...
            const friendsCount = await prisma_1.prisma.friendship.count({
                where: {
            ...
            const pending = await prisma_1.prisma.friendRequest.findFirst({
                where: {
                    status: "PENDING",
                    OR: [
    ...
    static async getPostsByProfileId(req, res) {
        try {
            const { profileId } = req.params;
    ...
    static async updateMyCover(req, res) {
        try {
            const userId = req.user.id;
