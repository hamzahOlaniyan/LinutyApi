...
function runFfmpeg(inputPath, outDir) {
    return new Promise((resolve, reject) => {
        ...
        const args = [
            ...
            "-hls_segment_filename", path_1.default.join(outDir, "240p_%03d.ts"),
            path_1.default.join(outDir, "240p.m3u8"),
            // 480p
            ...
            "-f", "hls",
            "-hls_time", "4", "-hls_playlist_type", "vod",
            "-hls_segment_filename", path_1.default.join(outDir, "480p_%03d.ts"),
            path_1.default.join(outDir, "480p.m3u8"),
            // 720p
            ...
            "-f", "hls",
            "-hls_time", "4", "-hls_playlist_type", "vod",
            "-hls_segment_filename", path_1.default.join(outDir, "720p_%03d.ts"),
            path_1.default.join(outDir, "720p.m3u8")
        ];
...
async function uploadDirToBucket(localDir, bucketPathPrefix) {
...
app.post("/transcode", async (req, res) => {
    try {
        ...
        // Download to /tmp
        const localInputPath = path_1.default.join(tmpDir, `input-${Date.now()}.mp4`);
        await rawFile.download({ destination: localInputPath });
        const hlsOutDir = path_1.default.join(tmpDir, `hls-${Date.now()}`);
        fs_1.default.mkdirSync(hlsOutDir);
        // Run ffmpeg to generate HLS variants
        await runFfmpeg(localInputPath, hlsOutDir);
        // Create master playlist
        const masterPath = path_1.default.join(hlsOutDir, "master.m3u8");
        const masterContent = [
            "#EXTM3U",
            "#EXT-X-VERSION:3",
        ...
        fs_1.default.writeFileSync(masterPath, masterContent, "utf8");
